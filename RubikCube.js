...
//				cubeXYZ[x][y][z].rotate();
    ...
    this.checkState = function() {
		...
		do {
			...
			tempColor = cubeXYZ[0][0][z].colorPositions[colPos];
			for(var x=0; x<3; x++) {
				for(var y=1; y<3; y++) {
					if(cubeXYZ[x][y][z].colorPositions[colPos] != tempColor) {
						wrong = true;
						break;
					}
				}
		...
		if(!wrong) {
			...
			do {
				...
				tempColor = cubeXYZ[x][0][0].colorPositions[colPos];
				for(var y=0; y<3; y++) {
					for(var z=1; z<3; z++) {
						if(cubeXYZ[x][y][z].colorPositions[colPos] != tempColor) {
							wrong = true;
			...
			if(!wrong) {
				...
				tempColor = cubeXYZ[0][y][0].colorPositions[colPos];
				for(var x=0; x<3; x++) {
					for(var z=1; z<3; z++) {
						if(cubeXYZ[x][y][z].colorPositions[colPos] != tempColor) {
							wrong = true;
